...
`deepseek_harness_desktop` starts on the configured Harness URL. Its small
native MoUI `D` button is hidden by default and can be enabled from **View →
Toggle Floating Button** to switch between that root and
`https://chat.deepseek.com/`. A primary click switches sites, a secondary click opens Settings,
...
Harness and Chat are backed by separate retained native WebViews. Chat is
preloaded in the background once settings are available; switching changes the
two placements' visibility instead of navigating a shared WebView. This keeps
the remote Chat page warm while preserving independent state for both roots.
